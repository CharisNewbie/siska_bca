...
if ($isEdit) {
    $stmt = $pdo->prepare("SELECT * FROM surat_kuasa WHERE id = ?");
    $stmt->execute([$editId]);
    $data = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        foreach ($uploadFields as $field) {
            ...
            if (isset($_FILES[$field]) && is_array($_FILES[$field]) && $_FILES[$field]['error'] !== UPLOAD_ERR_NO_FILE) {
                ...
                // Pertahankan file lama saat edit, atau null jika baru
    ...
    try {
        if ($isEdit) {
            ...
            // VALIDASI USER ID (pastikan user ada di database)
            if ($userId) {
                $checkUser = $pdo->prepare("SELECT id FROM users WHERE id = ? AND status = 'active'");
                $checkUser->execute([$userId]);
                if (!$checkUser->fetch()) {
                    // User tidak ditemukan atau tidak aktif
                    error_log('[SISKA] User ID ' . $userId . ' tidak valid untuk foreign key');
                    $userId = null;
                }
            }
            
            // Jika tidak ada user valid, gunakan admin ID 1 sebagai fallback
            if (!$userId) {
            ...
            $stmt->execute([
                ...
                $userId, // Sekarang aman
            ]);
            
            $newId = $pdo->lastInsertId();
            auditLog($pdo, 'tambah', 'surat_kuasa', (int) $newId);
...
<form method="POST" enctype="multipart/form-data" novalidate id="formSurat">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1.25rem;">
        ...
        <div>
            <div class="card-siska" style="margin-bottom:1.25rem;">
                <div class="card-header">
